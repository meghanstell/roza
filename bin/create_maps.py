...
class CoolHandler(MapHandler):
    ...
    def create_chart(self, strata=ChartStrataType.AIR, variable=ChartVariableType.TEMPERATURE, measurement=ChartMeasurementType.FREQUENCY, cache=True):
        ...
        title_strata='Airborne ' if strata in [ChartStrataType.AIR] else 'Ground '
        if variable in [ChartVariableType.ARDT]:
            title_strata=''
        _title_var_dd = {
            ChartVariableType.TEMPERATURE: 'Temperature',
            ChartVariableType.LWC: 'LWC',
            ChartVariableType.TLWC: 'Seedable Conditions',
            ChartVariableType.ARDT: 'Atmospheric River',
            ChartVariableType.TLWCAR: 'Seedable Conditions & Atmospheric River'}
        
        # title_variable='Temperature' if variable in [ChartVariableType.TEMPERATURE] else 'LWC'
        title_variable=_title_var_dd.get(variable)
        title_range = 'Nov - Apr' if data_range in [ChartRangeType.SEASON] else data_range.name.capitalize()
        title_detail=f'{title_strata}{measurement.name.capitalize()} of Seedable Conditions ({title_range})' if ((measurement == ChartMeasurementType.FREQUENCY) and (variable==ChartVariableType.TLWC)) else f'{title_strata} {title_variable} {measurement.name.capitalize()} ({title_range})'
        title=f'{self.model}\n{title_detail}'
    ...
    def _create_charts(self):
        ...
        _desired_charts=[
            ...
            #     measurement=ChartMeasurementType.FREQUENCY),
            dict(strata=ChartStrataType.AIR, 
                variable=ChartVariableType.TLWCAR, 
                measurement=ChartMeasurementType.FREQUENCY),
            dict(strata=ChartStrataType.AIR, 
                variable=ChartVariableType.ARDT, 
                measurement=ChartMeasurementType.FREQUENCY),
...
class CacheHandler(MapHandler):
    ...
    def create_chart(self, strata=ChartStrataType.AIR, variable=ChartVariableType.TEMPERATURE, measurement=ChartMeasurementType.FREQUENCY, data_range=ChartRangeType.SEASON):
        ...
        if range in [ChartRangeType.MAR012017]:
            _data_range_name=f'{range.value}'
        else:
            _data_range_name=data_range.name
        ...
        _cache_name = output_name
        if variable in [ChartVariableType.IVTAR]:
            _cache_name = f"RZ_{model_name}_{data_range.name}_{strata.name}_{ChartVariableType.IVT.name}_{measurement.name}"
        if variable in [ChartVariableType.ARDT]:
            if measurement in [ChartMeasurementType.AVERAGE]:
                _cache_name = f'RZ_{model_name}_{data_range.name}_{strata.name}_{ChartVariableType.ARDT.name}_{ChartMeasurementType.FREQUENCY.name}'
        _cache_dir=makedirs(os.path.join(root_dir, 'cache', _cache_name))
        _cache_file=os.path.join(_cache_dir, f'{_cache_name}.nc')
        ...
        # # Set the common contour ranges for the data types being mapped
        _cont_kwargs={
            (ChartVariableType.TAR, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 10, 11),),
            (ChartVariableType.TLWCAR, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 10, 11),),
            (ChartVariableType.ARDT, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 25, 11),),
            (ChartVariableType.TLWC, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 50, 11),),
            (ChartVariableType.LWC, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 50, 11),),
            (ChartVariableType.TEMPERATURE, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 50, 11),),
            (ChartVariableType.IVT, ChartMeasurementType.AVERAGE): DotMap(levels=np.linspace(min(min(data_limits), 0), 250, 11),),
            (ChartVariableType.LWC, ChartMeasurementType.AVERAGE): DotMap(levels=np.linspace(min(min(data_limits), 0), 0.1, 11),),
            (ChartVariableType.LWCAR, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 25, 11),),
            (ChartVariableType.ARDT, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 10, 11),),
            (ChartVariableType.TAR, ChartMeasurementType.FREQUENCY): DotMap(levels=np.linspace(min(min(data_limits), 0), 10, 11),),
            # (ChartVariableType.IVT, ChartMeasurementType.AVERAGE),
            (ChartVariableType.IVTAR, ChartMeasurementType.AVERAGE): DotMap(levels=np.linspace(min(min(data_limits), 0), 250, 11),),
            # (ChartVariableType.ARDT, ChartMeasurementType.AVERAGE),
            # (ChartVariableType.TLWCAR, ChartMeasurementType.FREQUENCY),
        ...
        _cont = None
        if (variable in [ChartVariableType.ARDT]) and (measurement in [ChartMeasurementType.AVERAGE]):
            hist = histogram(_fvar, bins=100)
            CS=plt.contour(
                self.dom_file.lon,
                self.dom_file.lat,
                _fvar,
                [hist.coords['data_bin'][hist.argmax()]],
                colors='blue',
                zorder=10,
                alpha=1, label='Average ARDT')
            ax.legend()
            ax.clabel(CS, 
                      inline=True, 
                      fontsize=10, 
        ...
        if (variable == ChartVariableType.IVTAR) and (measurement==ChartMeasurementType.AVERAGE):
            _data_name = f'RZ_{model_name}_{data_range.name}_{strata.name}_{ChartVariableType.ARDT.name}_{ChartMeasurementType.FREQUENCY.name}'
            _cache_dir=makedirs(os.path.join(root_dir, 'cache'))
            _cache_file = os.path.join(_cache_dir, _data_name, f'{_data_name}.nc')
            ds = xr.open_dataset(_cache_file)

            _data=ds['data']
            # display(_data)
            # ds['data'].plot.hist()
            # try:
            hist = histogram(ds['data'], bins=100)
        ...
        if _cont is not None:
            ...
            _cbar = plt.colorbar(
                ...
                shrink=0.9)
            _cbar.ax.tick_params(size=1, labelsize=14)
            _cbar_label=r'$Frequency\ \left(\%\right)$' if measurement==ChartMeasurementType.FREQUENCY else (r'$Temperature\ \left(^{\circ}C\right)$' if variable == ChartVariableType.TEMPERATURE else( r'$IVT\ \left(\frac{kg}{ms}\right)$' if variable==ChartVariableType.IVT else r'$LWC\ \left(\frac{g}{kg}\right)$'))
            _cbar.set_label(_cbar_label, fontsize = 20)
        ...
        title_strata='Airborne ' if strata in [ChartStrataType.AIR] else 'Ground '
        if variable in [ChartVariableType.ARDT, ChartVariableType.IVT]:
            title_strata=''
        _title_var_dd = {
            ChartVariableType.TEMPERATURE: 'Temperature',
            ChartVariableType.LWC: 'LWC',
            ...
            ChartVariableType.ARDT: 'Atmospheric River',
            ChartVariableType.TLWCAR: 'Seedable Conditions & Atmospheric River',
            ChartVariableType.IVT: 'Integrated Water Vapor Transport',
            ChartVariableType.IVTAR: 'Integrated Water Vapor Transport & Atmospheric River Mean',
            ChartVariableType.TAR: 'Temperature Criteria & Atmospheric River',
            ChartVariableType.LWCAR: 'LWC Criteria & Atmospheric River',
            }
        
        # title_variable='Temperature' if variable in [ChartVariableType.TEMPERATURE] else 'LWC'
        title_variable=_title_var_dd.get(variable)
        if data_range in [ChartRangeType.MAR012017, ChartRangeType.MAR182017]:
            _match = re.match(r'^(?P<year>\d{4})(?P<month>\d{2})(?P<day>\d{2})$', f'{data_range.value}')
            _year=_match.groupdict().get('year')
            _month=_match.groupdict().get('month')
            _day=_match.groupdict().get('day')
            _data_datetime=datetime.datetime(int(_year), int(_month), int(_day))
            title_range = f"{_data_datetime:%B} {_data_datetime:%d}, {_data_datetime:%Y}"
        else:
            title_range = 'Nov - Apr' if data_range in [ChartRangeType.SEASON] else data_range.name.capitalize()
        title_detail=f'{title_strata}{measurement.name.capitalize()} of Seedable Conditions ({title_range})' if ((measurement == ChartMeasurementType.FREQUENCY) and (variable==ChartVariableType.TLWC)) else f'{title_strata} {title_variable} {measurement.name.capitalize()} ({title_range})'
        title=f'{self.model}\n{title_detail}'
    ...
    def _create_charts(self):
        ...
        _desired_charts=[
            dict(strata=ChartStrataType.AIR, 
                ...
                measurement=ChartMeasurementType.FREQUENCY),
            # dict(strata=ChartStrataType.AIR, 
            #     variable=ChartVariableType.LWCAR, 
            #     measurement=ChartMeasurementType.FREQUENCY),
            # dict(strata=ChartStrataType.AIR, 
            #     variable=ChartVariableType.ARDT, 
            #     measurement=ChartMeasurementType.FREQUENCY),
            # dict(strata=ChartStrataType.AIR, 
            #     variable=ChartVariableType.TAR, 
            #     measurement=ChartMeasurementType.FREQUENCY),
            # dict(strata=ChartStrataType.AIR, 
            #     variable=ChartVariableType.IVT, 
            #     measurement=ChartMeasurementType.AVERAGE),
            # dict(strata=ChartStrataType.AIR, 
            #     variable=ChartVariableType.IVTAR, 
            #     measurement=ChartMeasurementType.AVERAGE),
            # dict(strata=ChartStrataType.AIR, 
            #     variable=ChartVariableType.ARDT, 
            #     measurement=ChartMeasurementType.AVERAGE),
        ...
        # _data_range=[ChartRangeType.MAR012017]
        _data_range=[ChartRangeType.MAR182017]
        # _data_range=[ChartRangeType.APRIL]
        for _range in _data_range:
            for dd in _desired_charts:
                try:
                    self.create_chart(**dd, data_range=_range)
                except Exception as exc:
                        self.logger.warning(f'Problem with create_chart({dd=}): {exc} ')
        
        # _var_range=[
        #     # ChartVariableType.LWC, ChartVariableType.TEMPERATURE, ChartVariableType.TLWC, 
        #     ChartVariableType.TLWCAR, ChartVariableType.ARDT, 
        #     ]
        # _data_range=[ChartRangeType.SEASON, ChartRangeType.JANUARY, ChartRangeType.FEBRUARY, ChartRangeType.MARCH, ChartRangeType.APRIL, ChartRangeType.NOVEMBER, ChartRangeType.DECEMBER]
        # _strata_range=[ChartStrataType.AIR, ChartStrataType.GROUND]

        # [self.create_chart(strata=_strata, variable=_variable, measurement=ChartMeasurementType.FREQUENCY, data_range=_range) for _strata in _strata_range for _variable in _var_range for _range in _data_range]
        # for _strata in _strata_range:
        #     for _range in _data_range:
        #         try:
        #             self.create_chart(strata=_strata, variable=ChartVariableType.IVT, measurement=ChartMeasurementType.AVERAGE, data_range=_range) 
        #         except Exception as exc:
        #             self.logger.warning(f'Problem with create_chart(strata={_strata}, data_range={_range}): {exc} ')
...
class LwcHandler(MapHandler):
    def create_chart(self, lat_index=None, lat=None):
        ...
        ax2.set_yticks([0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30])
        ...
        _lat_index_label={50:'Cross Section 1 (46.5, -122 to 46.4 to -118.5)', 60:'Cross Section 2 (46.8, -122 to 46.8 to -118.5)', 70:'Cross Section 3 (47.5 , -122 to 47.5,-118.5)'}
        _prefix=f'Vertical Distribution of Airborne & Ground-based LWC ( {title_range} )\n'
        # _tex=r'$\left(\ Latitude \  ' + f'{lat}' + r' \degree \ N\right)$'
        _tex=_lat_index_label.get(lat_index)
...
def animate_wind(ds_3d=None, domain=None, scope=None, **kwargs):
    ...
    # python bin/create_maps.py --sv-file data/ncfiles/sv/SV_2010_02.nc --dom-file data/wrfconstants_usgs404_roza.nc lwc