...
# INDIR3D="/glade/derecho/scratch/meghan/Roza/data/CONUS404/orig"
INDIR3D="/glade/campaign/collections/rda/data/d559000"
# outdir="/glade/derecho/scratch/meghan/Roza/data/CONUS404/cut/${YEAR}/${MONTH}"
outdir="/glade/campaign/ral/hap/meghan/Roza/data/CONUS404/cut/${YEAR}/${MONTH}"
outdir2="/glade/campaign/ral/hap/meghan/Roza/data/CONUS404/cut2/${YEAR}/${MONTH}"
...
function stagger_3d_uv(){
    infile=$1
    outfile=$(basename ${infile})
    outpath="${outdir2}/${outfile}"
...
function handle_3d_stagger_uv(){
    ...
    for infile in `find ${INDIR3D} -name "wrf3d_d01_${YEAR}-${MONTH}-*.nc" | sort`; do
        ...
        if [[ $(( $_count % 10 )) -eq 0 ]]; then
            ...
            _info "handle_3d_uv.${YEAR}.${MONTH}" "${CYAN}3D${RESET} ${YELLOW}-->${RESET} ${GREEN}${_pcnt}%${RESET}"
        fi
        stagger_3d_uv ${infile}
    done
    _success "handle_3d_uv.${YEAR}.${MONTH}" "Done"
}

function replace_3d_uv(){
    # ncks -x -v variable_name input_file.nc output_file.nc
    varuv="U,V"
    infile=$1
    outdir_3d="${outdir}"
    outfile=$(basename ${infile})
    outpath="${outdir_3d}/${outfile}"
    uv_file="${outdir2}/${outfile}"

    # _info "handle_3d.${YEAR}.${MONTH}" "Output file: ${YELLOW}${outpath}${RESET}"
    ncks -O -x -vU,XLAT_U,XLONG_U,V,XLAT_V,XLONG_V ${outpath} ${outpath}
    ncks -A -vU,V ${uv_file} ${outpath}
}

function handle_3d_uv(){
    outdir_3d="${outdir}"
    mkdir -p ${outdir_3d}
    _total=$(find ${INDIR3D} -name "wrf3d_d01_${YEAR}-${MONTH}-*.nc" | wc -l)
    _count=0
...
function handle_2d(){
    outdir_2d="${outdir}/2d"
    mkdir -p ${outdir_2d}
    _total=$(find ${INDIR3D} -name "wrf2d_d01_${YEAR}-${MONTH}-*.nc" | wc -l)
    _count=0
    for infile in `find ${INDIR3D} -name "wrf2d_d01_${YEAR}-${MONTH}-*.nc" | sort`; do
        _count=$(( $_count + 1 ))
        if [[ $(( $_count % 10 )) -eq 0 ]]; then
            _pcnt=$(echo "scale=2 ; $_count*100 / $_total" | bc)
...
function handle_const(){
    infile=${CONST}
    outdir_const="${outdir}"
    outfile="wrfconstants_usgs404_roza.nc"
