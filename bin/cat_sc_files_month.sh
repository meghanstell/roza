...
function concat_scfiles_month(){
    # Seeding_criteria_2003-03-19_230000.nc
    month=${1}
    cat_cmd="ncrcat --hst"
    outFile=${catDir}/SV_${month}.nc

    _info    "concat_scfiles_month.${month}" "Concatenating Seeding Criteria Files for Month ${month}"
    
    ${cat_cmd} ${monthlyDir}/SV_*_${month}.nc ${outFile}
    
    _success "concat_scfiles_month.${month}" "Month SV File located at ${outFile}"
}

concat_scfiles_month ${MONTH}
